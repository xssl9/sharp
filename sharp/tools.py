...
Отвечай кратко, 1-2 предложения, живым разговорным языком. Обращайся «сэр».
Говори спокойно и чуть медленнее обычного. Всегда заканчивай начатое предложение;
не обрывай речь на середине мысли.
...
Микрофон поступает к тебе непрерывным Live-потоком. Для НОВОЙ команды пользователь
обращается по имени в той же фразе. Считай обращением варианты «Шарп», «шар»,
«шарк», «sharp» и близкие ошибки распознавания. Если во входной реплике нет
обращения и это не ответ на твой вопрос, полностью проигнорируй её: не произноси
НИЧЕГО, даже «молчу», не вызывай инструменты и не меняй состояние. Исключение —
короткий ответ на твой непосредственно предыдущий вопрос (например «первая», «да»,
«отмена»): его принимай без повторения имени.
Если пользователь сказал только имя без команды, коротко ответь «Слушаю, сэр.» и
прими следующую короткую реплику как продолжение без повторения имени.
Не проси нажимать клавиши и не проси сначала отдельно сказать имя: пользователь
может говорить одной фразой, например «Шарп, открой Firefox».
...
- Открыть программу → open_application. Этот инструмент не требует shell-доступа.
- Выполнить команду и увидеть её exit_code/stdout/stderr → run_terminal. После вызова
  кратко перескажи пользователю фактический вывод, не выдумывай успешный результат.
- Установить программу через pacman → сначала вызови prepare_package_install, затем
  назови пакет и голосом спроси подтверждение. Только на СЛЕДУЮЩЕЙ реплике после
  явного «да/установи/подтверждаю» вызови install_package с тем же пакетом.
  Никогда не устанавливай пакет через run_terminal.
  Если точное имя пакета неизвестно, сначала найди его через run_terminal("pacman -Ss запрос").
...
Для действий сначала сразу вызови инструмент, затем коротко скажи результат. Не трать
отдельный голосовой ответ на промежуточное подтверждение."""
...
TOOLS: list[types.FunctionDeclaration] = [
    types.FunctionDeclaration(
        ...
        description="Открыть установленную графическую программу независимо от shell-доступа.",
        parameters=_schema(
            {"name": {"type": "STRING", "description": "Команда или название программы"},
             "arguments": {"type": "STRING", "description": "Необязательные аргументы"}},
            ["name"],
        ),
    ),
    types.FunctionDeclaration(
        name="run_terminal",
        description="Выполнить команду Bash и получить настоящий exit_code, stdout и stderr.",
        parameters=_schema(
            {"command": {"type": "STRING", "description": "Команда как в терминале"}},
            ["command"],
        ),
    ),
    types.FunctionDeclaration(
        name="prepare_package_install",
        description="Подготовить установку пакета и потребовать отдельное голосовое подтверждение.",
        parameters=_schema(
            {"package": {"type": "STRING", "description": "Точное имя одного пакета pacman"}},
            ["package"],
        ),
    ),
    types.FunctionDeclaration(
        name="install_package",
        description="Подтвердить ранее подготовленную установку после отдельного согласия пользователя.",
        parameters=_schema(
            {"package": {"type": "STRING", "description": "Точное имя одного пакета pacman"}},
            ["package"],
        ),
...
def execute(name: str, args: dict) -> dict:
    ...
    try:
        if name == "open_application":
            return {"result": commands.open_app(a.get("name", ""), a.get("arguments", ""))}
        if name == "run_terminal":
            return {"result": commands.run_terminal(a.get("command", ""))}
        if name == "prepare_package_install":
            return {"result": commands.prepare_package_install(a.get("package", ""))}
        if name == "install_package":
            return {"result": commands.install_package(a.get("package", ""))}
        if name == "open_url":
